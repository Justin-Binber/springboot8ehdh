...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/fuwuyuan/yuangonggonghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    yuangonggonghaoChange () {
      this.$http({
        url: `follow/fuwuyuan/yuangonggonghao?columnValue=`+ this.ruleForm.yuangonggonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `diandanpeisong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.diandanpeisongCrossAddOrUpdateFlag = false;
						 this.parent.search();
