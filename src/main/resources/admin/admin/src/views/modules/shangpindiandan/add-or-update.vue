<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="厢房号" prop="xiangfanghao">
          <el-select :disabled="ro.xiangfanghao" v-model="ruleForm.xiangfanghao" placeholder="请选择厢房号">
            <el-option
                v-for="(item,index) in xiangfanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      this.$http({
		...
		if(json.yonghuxingming!=''&&json.yonghuxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.shangpinfenleiOptions = "酒水,零食,水果".split(',')
            this.$http({
              url: `option/xiangfangyuyue/xiangfanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.xiangfanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shangpindiandan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shangpindiandanCrossAddOrUpdateFlag = false;
						 this.parent.search();
