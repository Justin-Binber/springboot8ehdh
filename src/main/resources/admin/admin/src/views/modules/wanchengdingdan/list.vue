      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            ...
            <el-button
              v-if="isAuth('wanchengdingdan','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="warning"
              icon="el-icon-s-data"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
            <el-button
              v-if="isAuth('wanchengdingdan','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}<i class="el-icon-s-data el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('wanchengdingdan','报表') && contents.btnAdAllIcon == 0"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
...
export default {
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        //  zongjine
        // xiangfanghao xiangfanghao
        // xcolumn ycolumn xiangfanghao zongjine
        var zongjineChart = this.$echarts.init(document.getElementById("zongjineChart"),'macarons');
        this.$http({
            url: `wanchengdingdan/value/xiangfanghao/zongjine`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].xiangfanghao);
                    yAxis.push(parseFloat((res[i].total)).toFixed(2));
                    pArray.push({
                    value: parseFloat((res[i].total)),
                    name: res[i].xiangfanghao
                    })
                    var option = {};
                    option = {
                        tooltip: {
